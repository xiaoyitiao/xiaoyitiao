...
智药伴支持将老人用药数据同步到 LeanCloud 云端，家属可通过家庭编号和查看密码远程查看。

#### 1. 注册 LeanCloud 账号

- 访问 LeanCloud 官网注册账号并创建应用。
- 在应用设置中获取「应用 ID」和「应用 Key」。
- 国内用户通常使用国内版，服务地址可不填；国际版用户需填写对应服务地址。
...
1. 开启「云端同步」。
2. 填写 LeanCloud 的「应用 ID」和「应用 Key」。
3. 设置「家庭编号」和「查看密码」，并告知家属。
4. 点击「立即同步到云端」。

开启云端同步后，老人每次添加药品、打卡或修改数据，系统会自动延迟同步到云端。
...
1. 填写老人提供的家庭编号和查看密码。
2. 配置相同的 LeanCloud 应用 ID 和 Key。
3. 点击「查看老人用药情况」，即可看到老人今日用药状态、漏服预警等信息。

> 注意：LeanCloud 免费版有请求次数和存储空间限制，适合家庭小规模使用。查看密码请妥善保管，不要泄露给无关人员。

### 拍照识别说明

拍照识别采用「前端本地 OCR + 智能解析」方案：

1. 使用 `Tesseract.js` 在浏览器内识别药品包装盒上的文字。
2. 优先匹配内置本地药品数据库，自动填充名称、剂量、时间、备注。
3. 若未匹配到本地药品，且已配置 AI 接口，则调用大模型从识别文字中智能提取药品信息。
4. 若以上方式均无法解析，将使用本地规则提取文字中的药品信息作为兜底。

首次使用时，识别需要加载中文语言包，请保持网络畅通。拍照时请尽量保证药品名称、规格、用法用量清晰可见。

如需更高准确率，可在 `js/ai.js` 中配置真实 AI 接口，由大模型辅助解析。
